...
void print_dns_message(const char* message, int msg_length) {
  ...
  // This prints out the whole text, so we should only print it out
  // if we want to see the whole message
  /*
  int i;
...
int main(int argc, char* argv[]) {
  ...
  char query[1024] = {static_cast<char>(0xAB),
                      static_cast<char>(0xCD), // ID
  ...
  *p++ = 0x01; //QCLASS
  ...
  // Check if query was correct or if we typoed
  print_dns_message(query, query_size);

  char read[1024];
  int bytes_received = recvfrom(socket_peer, read,
                                1024, 0, 0, 0);
